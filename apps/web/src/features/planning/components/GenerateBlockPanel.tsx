 ...
 */
export function GenerateBlockPanel({ situationId, subjectId, subnivel }: GenerateBlockPanelProps) {
    ...
    draftBlock.mutate(
      {
        ...
        competencyIds: isCompetencyModel ? competencyIds : undefined,
      },
      {
        onSuccess: (result) => {
    ...
    <Card className="space-y-4 border-primary/30 bg-primary/5 p-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Elige {isCompetencyModel ? 'la(s) competencia(s)' : 'la(s) destreza(s)'} y cuántas semanas dura este bloque —
          se crean y llenan todas las semanas automáticamente, sin entrar una por una.
        </p>
      </div>
